...
def ivim_data_load_cnn(path, norm_type='mean'):
    # load simulations
    Dataset = loadmat(path)
    ...
    #X_train_2D = X_train_2D[:, 1:, :, :] #solo per il topo
    #b_values = b_values[1:]
    ...
    if norm_type == 's0':
        ...
        for i in range(b_values.shape[0]-1):  # togliere background
            test_matrix = X_train_2D_scaled[:, i, :, :]
            test_matrix = np.where(D_true > 0, test_matrix, 0)
            X_train_2D_scaled[:, i, :, :] = test_matrix  # masking signal background at 0
        b_values = b_values[1:]
        ...
        # Iterate over each sample in the batch
        for i in range(X_train_2D_scaled.shape[0]):
            # Extract bval=0 slice for the current sample
            b0 = X_train_2D[i, 0, :, :]  # This is the b0 image (bval=0)

            roi_mask = D_true[i, :, :] > 0  # Mask where D_true > 0
            # Find the min and max values of b0 (over width and height)
            ...
            # Perform Min-Max scaling for the entire sample (using the min/max of b0)
            for j in range(b_values.shape[0]):  # Iterate over all bvals
                # Extract the current b-value slice (across the width and height)
                bval_slice = X_train_2D[i, j, :, :]
        ...
        for i in range(b_values.shape[0]):  # togliere background
            test_matrix = X_train_2D_scaled[:, i, :, :]
            test_matrix = np.where(D_true > 0, test_matrix, 0)
            X_train_2D_scaled[:, i, :, :] = test_matrix  # masking signal background at 0


    return X_train_2D_scaled, b_values, D_true, f_true, Dstar_true
...
class IVIMDataset(data.Dataset):
    def __init__(self, X, D_true, f_true, Dstar_true, bounds, phase='train', device="cpu"):
        """
        Args:
            X (numpy array or torch.Tensor): Input signal data of shape (N, C, H, W)
            D_true, f_true, Dstar_true: Ground truth maps of shape (N, 1, H, W)
        """
        self.device = device
        self.X = torch.tensor(X, dtype=torch.float32).to(device)
        self.D_true = torch.tensor(D_true, dtype=torch.float32).to(device)
        self.f_true = torch.tensor(f_true, dtype=torch.float32).to(device)
        self.Dstar_true = torch.tensor(Dstar_true, dtype=torch.float32).to(device)
        self.phase = phase
    ...
    def __getitem__(self, idx):
        ...
        d = self.D_true[idx]
        f = self.f_true[idx]
        dp = self.Dstar_true[idx]

        if self.phase == 'train':
            x, d, f, dp = self.apply_transformations(x, d, f, dp)

        d, f, dp = scale_params(d, f, dp, self.bounds)
        return x, d, f, dp
    ...
    def setup_transformations(self):
        ...
        augmentation_transforms = T.Compose([
            T.RandomRotation(degrees=180, interpolation=T.InterpolationMode.NEAREST),
            T.RandomAffine(degrees=0, shear=10,translate=(0.1, 0.1), interpolation=T.InterpolationMode.NEAREST),  # Apply random shear
            T.RandomHorizontalFlip(),  # Random horizontal flip
            T.RandomVerticalFlip()])  # Random vertical flip

        self.transform_spatial = augmentation_transforms

    def apply_transformations(self, x, d, f, dp):
        # Concatenate input and targets along the channel axis
        combined = torch.cat([x, d.unsqueeze(0), f.unsqueeze(0), dp.unsqueeze(0)], dim=0)  # Shape: (C+3, H, W)

        # torchvision transforms expect a PIL image or torch tensor in (C, H, W) on CPU

        # Apply transform
        transformed = self.transform_spatial(combined)

        # Split the combined tensor back
        x_aug = transformed[:x.shape[0], :, :].to(self.device)
        d_aug = transformed[x.shape[0], :, :].to(self.device)
        f_aug = transformed[x.shape[0] + 1, :, :].to(self.device)
        dp_aug = transformed[x.shape[0] + 2, :, :].to(self.device)

        return x_aug, d_aug, f_aug, dp_aug
